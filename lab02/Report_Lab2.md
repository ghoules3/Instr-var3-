...
### Бизнес-вывод  
Полученные результаты показывают, что популярность Keanu Reeves в 90-е годы формировалась не равномерно по всему набору фильмов, а концентрировалась вокруг наиболее сильных и медийно заметных проектов. Наибольший вклад в обсуждаемость актёра внёс фильм **The Matrix**, который существенно опережает остальные фильмы по числу комментариев.  
  
С продуктовой точки зрения это означает, что при формировании тематических подборок, рекомендательных блоков и рекламных кампаний для аудитории, интересующейся фильмами 90-х, наиболее эффективно использовать фильмы-лидеры обсуждаемости как точки входа. В данном случае **The Matrix**, **Bram Stoker's Dracula**, **The Devil's Advocate** и **Speed** могут рассматриваться как ключевые фильмы для привлечения внимания к контенту с участием Keanu Reeves.  
  
Соответственно ответ на поставленный в варианте вопрос следующий: **Keanu Reeves был популярным актёром 90-х годов, а его популярность особенно усилилась к концу десятилетия благодаря наиболее обсуждаемым и резонансным фильмам.**  
  
# Выводы  
В ходе выполнения практической работы был реализован полиглотный подход к хранению данных для бизнес-кейса.  
  
## Почему для каталога был выбран MongoDB  
MongoDB подходит для хранения каталога контента, так как:  
-использует документную модель хранения  
-поддерживает вложенные массивы и объекты  
-позволяет гибко изменять структуру данных без жёсткой схемы  
MongoDB удобно использовать для хранения метаданных фильмов, жанров, актёров и других характеристик контента.  
  
## Почему для графовых связей был выбран GraphDB  
GraphDB подходит для поиска связей и рекомендательных сценариев, так как:  
-хранит RDF-граф  
-поддерживает язык SPARQL  
-позволяет анализировать семантические связи между фильмами и актёрами  
Использование GraphDB удобно для поиска взаимосвязей между сущностями и построения рекомендательных сценариев на основе графовой модели.  
  
## Почему аналитика выполнялась отдельно  
JupyterLab и Python использовались как аналитический слой, позволяющий:  
-загружать результаты запросов  
-строить визуализации  
-выполнять бизнес-интерпретацию результатов  
  
## Преимущества Polyglot Persistence  
Использование разных технологий позволило применить сильные стороны каждой из них:  
-**MongoDB** гибкое хранение метаданных фильмов  
-**GraphDB** анализ семантических связей  
-**JupyterLab / Python** визуализация и аналитический вывод  
  
Таким образом, подход **Polyglot Persistence** позволяет эффективнее решать разнородные задачи, чем использование одной универсальной СУБД для всех этапов обработки данных.
