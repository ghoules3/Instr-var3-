...
SparkSession создан в JupyterLab:
...
- проверены/удалены полные дубликаты строк
...
### 7.2 Heatmap
Построена heatmap: количество товаров по ABC-категории и диапазонам модельного остатка:
![heatmap](screenshots/16_heatmap.png)

График сохранён в HDFS:  
`/user/hadoop/lab_01_var3/output/plots/heatmap_stock_by_abc.png`

Heatmap показывает количество товаров в категориях A/B/C по диапазонам остатка. В категориях A и B видна высокая доля дефицита (<0), что говорит о потребности в более частом пополнении или увеличении страхового 
запаса для высокооборотных/высокодоходных товаров. Категория C более равномерно распределена по диапазонам остатков.
...
- В Spark SQL рассчитаны спрос, точка заказа, критически низкий запас и коэффициент оборачиваемости
